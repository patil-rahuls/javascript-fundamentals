...
// DOM tree is generated from an HTML document, which we can
// then interact with, using javascript.
// We write javascript code to create, modify and delete
// HTML elements, set styles, set classes and attributes, and
// listen and respond to the events that we attach to them.


// 2.  Document API OR DOM API ?
// Allows you to modify a DOM tree in any way you want.
// We can edit the DOM of a document using JavaScript to access
// the 'document' property of the global object.


// 3. What are NodeList and HTMLCollection ?
// Both the NodeList and HTMLCollection contain HTML elements
// that we query from javascript. The HTMLCollection changes in
// real time when we make changes in the DOM.
// But the same does not happen with NodeList.


//  1. document.querySelector()
// returns a SINGLE (first matching) element.
// We provide CSS selectors to these methods.
//  Selecting by class
document.querySelector('.messageDiv');
//  Selecting by id
document.querySelector('#message');

// We can provide multiple selectors to this method
...
// When an event occurs, Javascript generates an object
// that contains all info about that event.
// We use that object to handle the event in the handler
// function. We pass that event object as an
...
//  17. getAttribute - Getting Non Standard Attributes
// like the ones defined by user.
...
loginBTN.src; // returns complete url 'http://...'
