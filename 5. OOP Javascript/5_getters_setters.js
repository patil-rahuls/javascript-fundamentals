...
// Condition: the setter method name should be same as the
// parameter's name in contructor() which need to be validated.
// Also, if valid, we need to set the value to new property,
// because we can't use the same property name because
...
// In the above code(first `this.format.test...` line)
// will give error because, the same class variable 'username'
// is being initialized by two methods: the constructor and
// the setter.
// This results in  "max stack call size exceed error".
//
// Hence inside setter, we assign the value to a new variable
// "_username".
...
// TIP: We can have more than one getters and setters in an
