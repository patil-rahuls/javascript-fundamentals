...
Threads can be seen like a to-do list to perform those tasks. `OS scheduler` picks which threads to execute first utilizing different cores available in the CPU.
...
These 2 Cores are called `Physical Cores`. Each physical core can process 2 threads at a time (Please correct me if I am wrong).

`Logical core` = `Physical Cores` x `Threads`
...
e.g. `crypto.pbkdf2()`, file-system reads and writes, etc which uses multi-threading.
