...
### 3. Execution -
The compiled Machine code gets executed immediately. The execution happens in Javascript Engine's Call Stack.
...
While the interpreter runs the code, a built-in monitor (aka profiler) watches how often different sections of code are executed.

> Code that runs a few times is called `warm` and code that runs repeatedly, especially in loops or frequently called functions, is called `hot`.
...
> All these JIT processes happen in special threads that we can't access which are completely separated from the main thread which executes code in the Call Stack.
