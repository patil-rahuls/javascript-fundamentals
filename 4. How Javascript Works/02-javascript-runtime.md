...
- **Queues** - _Datastructure that holds callback functions of the asynchronous tasks that are to be executed._
   ...
   - **_Render Queue_** - _aka Animation Task Queue, handles the browser's visual repaint cycle. Callbacks of **requestAnimationFrame()** go here._
   - **_Macrotask Queue_** - _aka Callback/Task Queue, holds callbacks of all the standard async operations initiated by Web APIs e.g._
   ...
   - **_Idle Task Queue_** - _Low priority Queue that runs only if browser's main thread is completely free. It takes callbacks of **requestIdleCallback()**._
...
- **Queues** - _Datastructure that holds callback functions of the asynchronous tasks that are to be executed._
   ...
   - **_Microtask Queue_** - _This queue contains two different queues:_
     - _**nextTick Queue** - holds the callbacks of the **process.nextTick()**_
     - _**Promise Queue** - holds the callbacks of **Promises**._

   - **_Timer Queue_** - _holds the callbacks of timer functions i.e. **setTimeout()** and **setInterval()**._

   - **_I/O Queue_** - _holds the callbacks of all the async methods such as those of **fs**, **http** modules etc._

   - **_Check/Immediate Queue_** - _holds the callbacks of **setImmediate()**._

   - **_Close Queue_** - _holds the callbacks of the **close** event of an async task._
...
◀️ [Compilation, Interpretation and JIT](01-JIT-compilation.md)  
