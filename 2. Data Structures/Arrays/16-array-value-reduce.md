...
`reduce()` boils down all the array values to one single value. Returns a single value NOT an array.
...
which is an accumulator.
   ...
   the iterations.
   On every iteration, a value needs to be returned
   which goes to the accumulator, it can be the 'acc' or
   any calculations with acc.
*/
```
...
, txn[0]);
// here the accumulator is the first item of the array.
