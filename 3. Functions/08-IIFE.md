...
// The trailing "()" executes this anonymous function
// instantly.
...
> _Ans. "Variables Scope".
We use IIFE to just keep some variables hidden in the function scope and not make it available in global scope hence preventing the global scope from getting polluted._
...
> Example: One great example can be the implementation of Modules system in JS. Behind the scenes, Modules are nothing but IIFEs.

```javascript
const CounterModule = (function () {
  // 1. PRIVATE VARIABLES AND FUNCTIONS
  // These cannot be accessed directly from the
  // outside world
  let count = 0;
  ...
  // 2. PUBLIC API
  // We return an object containing the methods
  // we WANT to expose.
  return {
    increment: function () {
      count++;
