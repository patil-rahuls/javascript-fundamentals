...
// booker() still has access to the variable 'users' which
// was declared in parent function parentFn().
...
// A closure ensures that a function doesn't lose connection to
// variables that existed at the function's creation scope.

// Formal explanation :
// It is the closed-over variable environment of the execution
// context in which a function was created, and which exists
...
// wherever it goes(from wherever it is being called).
// This backpack has all the variables that were present in
...
// It gives the function details. where it shows the 'scopes'
// property which is the variable environment.
// We can see which variables exist in that VE and
// that the variables are changing in it based on the no.
// of times I am calling booker().
...
// The above line means that we have the variable 'users'
// is available from parentFn’s execution context
// In Scopes[3], 3 is the number of times the child function
// has been called.
//////////////////////////////////////////////////////////////
...
// Example : 2 - Timer Fn.
// We don't always need to return a function to
// observe a closure.
...
// Observe that the callback function of the timer was
// executed completely independent of the test().
// But still the callback function was able to access all
// the variables which were created in the test().
...
// But the eventlistner's callback function will be on call
// stack and waiting for the 'click' event to occur and even
// though the IIFE has finished executing, the callback
// function will still have access to the 'header' element.
