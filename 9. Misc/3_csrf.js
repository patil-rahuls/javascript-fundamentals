...
Ability of an evil website/app to perform requests to
server on victim's behalf.

This could be done by hiding a form behind a click on a
link/image. A form submission can be mimicked by an
attacker on page load, along with the victim's cookies to
make changes in victim's data on that vulnerable website.
...
get sent along with that POST request, and it will appear
to be a valid requestfor the 'xyz.com' server).
...
The underlying problem is that a cookie is sent with a
request regardless of where (which origin, ie. domain) the
request comes from, the only thing that counts is where it
goes to.

So if there is a user logged on to an application (having a
session cookie), an attacker may try and get this user to
visit his malicious website, from where the attacker
...
There is one important thing playing a role, the same
origin policy (SOP) in browsers.
A bit simplified, it means if something is downloaded from
one domain (or more precisely: origin), then another
domain will not have access to it.
...
1. Synchronizer tokens
   ...
   form to acquire a token, but then he would need
   credentials.
...
2. Double posting [Angular JS method]
   A different strategy is to generate a token, and set it
   as a cookie for the client.
   The client then reads the token from the cookie, and
   sends the same token as a request header as well.
   The server only compares whether the request header and
   the cookie contain the same token.

   This works, because an attacker cannot read the
   application token from a cookie set by a different
   origin (see SOP above), but the application on the
   legitimate domain can. So the client sending the request
   effectively proved that it is running on the legitimate
   application domain.
...
client as a cookie (non http-only) (i.e. in a Set-Cookie
header), and then Javascript in the client scrape it out
of the cookie and attach it as a separate XSRF-TOKEN header
...
In addition to using Double posting we should use sameSite
cookie type. Also, In order to prevent our page being
loaded into an iframe, we should use X-Frame-Options
headers.
X-Frame-Options: DENY
X-Frame-Options: SAMEORIGIN

Instead of sending this header manually in each requests,
we can configure our server to send this header
automatically with all requests,
https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/X-Frame-Options

Also, to make your app even more secure, you can disable
the feature of opening any page in another tab while the
1st tab is already open. i.e. ensuring that only one tab
...
url and pastes it in a new tab, it will fail/or log the
user out for safety.

When the user clicks any links, it should be a form
submitted to server along with CSRF token which will then
be validated by the server and the respective page will be
served on that route.
I have never seen this kind of security implementation
anywhere, may be because it ruins the UX.
